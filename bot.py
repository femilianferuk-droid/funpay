...
def create_platega_payment(amount: float, description: str) -> Optional[Dict]:
    ...
    try:
        ...
        data = {
            ...
            'amount': amount + (amount * 0.05),  # Добавляем комиссию 5%
            'currency': 'RUB',
            'description': description
        }
        
        response = requests.post(
            f'{PLATEGA_BASE_URL}/payments',
            headers=headers,
            json=data
        )
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Platega error: {response.text}")
            return None
...
def create_crypto_payment(amount: float, description: str) -> Optional[Dict]:
    """Создание платежа через Crypto Bot"""
    try:
        headers = {
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Crypto Bot error: {response.text}")
            return None
    ...
    if user_id == ADMIN_CHAT_ID and context.user_data.get('adding_product'):
        ...
        if stage == 'name':
            ...
            try:
                ...
                context.user_data['product_stage'] = 'category'
...
async def handle_category_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("cat_select_"):
        ...
        add_product(
            ...
            product_type="Товар",  # Можно добавить выбор типа если нужно
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(button_handler, pattern="^(shop_categories|help|admin_panel|admin_stats|admin_broadcast|admin_add_product|admin_manage_categories|admin_add_category|back_to_shop|back_to_admin)"))
    application.add_handler(CallbackQueryHandler(handle_category_selection, pattern="^cat_select_"))
    application.add_handler(CallbackQueryHandler(button_handler, pattern="^(category_|product_|buy_|pay_|check_|admin_delete_category_).*"))
